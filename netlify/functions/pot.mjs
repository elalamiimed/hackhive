...
// the shared pot. one store, every visitor reads the same list.
import { getStore } from "@netlify/blobs";

const ACCESS_CODE = "HITSZ2025"; // ← the real code lives HERE now. change it here.
const SEED = 20250401;           // fixed seed → everyone gets identical pairs

const store = getStore("hackhive-pot");

// Shanghai is UTC+8, no DST. read wall-clock via getUTC* on a shifted date.
function matchAtMs() {
  const sz = new Date(Date.now() + 8 * 3600e3);
  const day = sz.getUTCDay();
  let diff = (5 - day + 7) % 7;   // days until Friday
  if (diff === 0) diff = 7;
  // Friday 00:00 Shanghai − 1h = Thursday 23:00 Shanghai, as absolute ms
  return Date.UTC(sz.getUTCFullYear(), sz.getUTCMonth(), sz.getUTCDate() + diff, 0, 0, 0)
         - 8 * 3600e3 - 3600e3;
}
...
function pairs(pot) {
  if (Date.now() < matchAtMs()) return null; // not matching time yet
  const s = seededShuffle(pot.map(p => p.name), SEED);
  const m = {};
  for (let i = 0; i + 1 < s.length; i += 2) { m[s[i]] = s[i + 1]; m[s[i + 1]] = s[i]; }
  if (s.length % 2) m[s[s.length - 1]] = null; // odd one out waits
  return m;
}
